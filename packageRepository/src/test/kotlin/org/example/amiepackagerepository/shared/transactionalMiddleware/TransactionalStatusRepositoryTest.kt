...
class TransactionalStatusRepositoryTest {
    ...
    fun `SET action - should return INTERNAL_SERVER_ERROR when github upload fails`() {
        ...
        doThrow(RuntimeException("Network error"))
            .`when`(simpleService).uploadFileData(any(), any(), any())
    ...
    fun `getDeviceStatuses - should return list of device statuses when records exist`() {
        val expectedStatuses = listOf(
            DeviceStatus(
                deviceKey = "dev-1", name = "Router", port = "8080", deviceEndpoint = "/api/v1", user = testUser
            ),
            ...
            DeviceStatus(
                ...
                name = "Switch",
                port = "9090",
                deviceEndpoint = "/api/v2",
                user = testUser
            )
        )
        `when`(deviceStatusRepository.findByUserUsername("john_doe")).thenReturn(expectedStatuses)
