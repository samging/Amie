...
class DriveControllerTest {
    ...
    @BeforeEach
    fun setUp() {
        // Inject the @Value property downloadPath using ReflectionTestUtils
    ...
    fun `listFiles - GET list-disk - returns file list string from DriveService`() {
        ...
        whenever(simpleService.listFiles(driveService)).thenReturn(expectedListString)
    ...
    fun `listFiles - GET list-disk - returns fallback message when drive is empty`() {
        ...
        whenever(simpleService.listFiles(driveService)).thenReturn(emptyMessage)
    ...
    fun `downloadFile - GET download - uses default filename 'welcome-message' when param omitted`() {
        ...
        doNothing().whenever(simpleService).downloadFile(eq(driveService), eq(defaultFileName), any())
    ...
    fun `downloadFile - GET download - downloads specified file successfully`() {
        ...
        doNothing().whenever(simpleService).downloadFile(eq(driveService), eq(fileName), any())
    ...
    fun `downloadFile - GET download - catches exception and returns failure message`() {
        ...
        whenever(simpleService.downloadFile(eq(driveService), eq(fileName), any()))
